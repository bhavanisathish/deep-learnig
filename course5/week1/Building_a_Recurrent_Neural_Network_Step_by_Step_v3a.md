...
def rnn_forward(x, a0, parameters):
    ...
    for t in range(T_x):
        ...
        xt = x
        a_next, yt_pred, cache = rnn_cell_forward(xt[:,:,t], a_next, parameters)
...
def lstm_forward(x, a0, parameters):
    ...
    for t in range(T_x):
        ...
        a_next, c_next, yt, cache = lstm_cell_forward(xt[:,:,t], a_next, c_next, parameters)
        # Save the value of the new "next" hidden state in a (≈1 line)
        a[:,:,t] = a_next
...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    dot = da_next*np.tanh(a_next)*ot*(1-ot)
...
```
    ...
    gradients["dxt"][1][2] = 3.20643227196
    gradients["dxt"].shape = (3, 10)
    gradients["da_prev"][2][3] = -0.0594752269189
    gradients["da_prev"].shape = (5, 10)
    ...
    gradients["dWo"][1][2] = 0.0963568560865
...
```
    ...
    gradients["dx"][1][2] = [-0.00252516  0.12033704 -0.23151989 -0.33130107]
    gradients["dx"].shape = (3, 10, 4)
    gradients["da0"][2][3] = -0.1227321131
    gradients["da0"].shape = (5, 10)
    ...
    gradients["dWc"].shape = (5, 8)
    gradients["dWo"][1][2] = 0.0342213069711
    gradients["dWo"].shape = (5, 8)
